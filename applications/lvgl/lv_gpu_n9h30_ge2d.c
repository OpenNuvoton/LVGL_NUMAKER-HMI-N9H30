...
void lv_draw_n9h30_ge2d_blend(lv_draw_ctx_t *draw_ctx, const lv_draw_sw_blend_dsc_t *dsc)
{
    ...
    blend_area_w = lv_area_get_width(&blend_area);
	  blend_area_stride = blend_area_w * (LV_COLOR_DEPTH / 8);

		word_aligned = ((blend_area_stride & 0x3) == 0) &&
									 ((blend_area.x1*(LV_COLOR_DEPTH/8) & 0x3) == 0) ? 1 : 0;

    //rt_kprintf("[%s] %d %d %d\n", 	__func__, lv_area_get_size(&blend_area), (blend_area_stride & 0x3), (blend_area.x1 & 0x2));
        ...
        lv_color_t color)
{
    ...
    //rt_kprintf("[%s] %d %08x] %dx%d %d %d\n", __func__, lv_area_get_size(fill_area), dest_buf, fill_area_w, fill_area_h, fill_area->x1, fill_area->y1 );
        ...
        const lv_color_t *src_buf, lv_coord_t src_stride, lv_opa_t opa)
{
    ...
    //rt_kprintf("[%s] %d %08x -> %08x] (x:%d y:%d, %dx%d) <stride src:%d dst:%d>\n", __func__, lv_area_get_size(dest_area), src_buf, dest_buf, dest_area->x1, dest_area->y1, dest_w, dest_h, src_stride, dest_stride);
